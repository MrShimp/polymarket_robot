...
class BTCAutoTrader:
    ...
    def terminate_current_strategy(self):
        """强制终止当前运行的策略"""
        if (
            self.current_strategy_process
            and self.current_strategy_process.poll() is None
        ):
        ...
        if self.current_strategy_process:
            if self.current_strategy_process.poll() is None:
                ...
                if return_code == 0:
                    ...
                    # 获取错误输出
                    try:
                        stdout, stderr = self.current_strategy_process.communicate(
                            timeout=5
                        )
                        if stderr:
