...
class BTC15MinStrategy:
    ...
    def is_valid_entry_time(self) -> Tuple[bool, str]:
        ...
        if beijing_time < min_entry_time:
            ...
            return False, f"距离可入场时间还有 {remaining:.0f} 秒 (需等待{self.min_time_after_start}分钟)"
    ...
    def check_price_movement(self, current_price: float) -> Tuple[bool, str, str]:
        ...
        if abs_diff >= effective_threshold:
            ...
            return True, f"价格波动 ${abs_diff:.2f} >= ${effective_threshold} (基准${self.baseline_price:,.0f})", direction
        else:
            return False, f"价格波动 ${abs_diff:.2f} < ${effective_threshold} (基准${self.baseline_price:,.0f})", "none"
    ...
    def log_minute_price_difference(self, current_price: float):
        ...
        if self.last_minute_log != current_minute:
            ...
            if self.baseline_price:
                ...
                # 检查是否满足入场阈值
                threshold_status = "✅ 满足" if abs_diff >= self.buffer_threshold else "❌ 不满足"
                
                self.log(f"📊 [{current_time.strftime('%H:%M')}] BTC: ${current_price:,.2f} | "
                        f"基准: ${self.baseline_price:,.2f} | "
                        f"差额: {direction}${abs_diff:.2f} | "
                        f"阈值${self.buffer_threshold}: {threshold_status}")
            else:
                self.log(f"📊 [{current_time.strftime('%H:%M')}] BTC: ${current_price:,.2f} | 基准价格未设置")
    ...
    async def check_new_interval(self):
        ...
        if self.current_interval != interval_start:
            self.log(f"📅 新的15分钟区间: {interval_start.strftime('%H:%M')}-{interval_end.strftime('%H:%M')}")
    ...
    def save_interval_data(self, interval_start: datetime, start_price: float):
        ...
        try:
            ...
            filename = os.path.join(data_dir, f"intervals_{datetime.now().strftime('%Y%m%d')}.csv")
    ...
    def check_stagnant_condition(self) -> Tuple[bool, str]:
        ...
        if max_change < self.stagnant_price_change:
            return True, f"30秒内最大变化 ${max_change:.2f} < ${self.stagnant_price_change}"
        
        return False, f"30秒内最大变化 ${max_change:.2f} >= ${self.stagnant_price_change}"
        ...
        while self.running and not self.stop_event.is_set():
            try:
                ...
                if not self.position:
                    ...
                    # 检查双向入场信号
                    should_enter, entry_side, entry_prob = self.should_enter_position(yes_prob_pct, no_prob_pct, direction)
                    
                    if should_enter:
                        self.log(f"🚀 入场信号: {entry_side.upper()} 概率{entry_prob:.1f}%, 方向{direction}, {price_msg}")
                    ...
                    self.log(f"📊 持仓监控: {self.position['side'].upper()} 概率{current_prob_pct:.1f}%, 盈利{profit_points:.1f}点 (卖出无时间限制)")
    ...
    async def exit_position(self, token_id: str, shares: float) -> bool:
        ...
        try:
            ...
            self.log(f"❌ 出场操作失败: {e}", "ERROR")
            return False
    
    def save_trade_record(self, market_id: str, position: Dict, exit_price: float, profit: float, exit_reason: str):
        """保存交易记录"""
        try:
            trade_record = {
                ...
                'interval': position.get('interval').isoformat() if position.get('interval') else None,
                'entry_time': datetime.fromtimestamp(position.get('entry_time')).isoformat(),
    ...
    async def start_strategy(self, market_id: str, amount: float = 10.0):
        ...
        self.log("=" * 60)
        self.log(f"📊 策略参数:")
        self.log(f"   交易时段: {self.trading_hours['start']}:00-{self.trading_hours['end']}:00 (北京时间)")
        ...
        self.log(f"   特殊止盈: {self.special_tp_threshold*100}% + {self.stagnant_time}秒横盘")
...
async def main():
    ...
    try:
        ...
        if len(sys.argv) >= 2:
            ...
            try:
                amount = float(input("💰 请输入交易金额 (USDC) [默认10]: ").strip() or "10")
                if amount <= 0:
                    print("❌ 金额必须大于0")
                    return
            except ValueError:
